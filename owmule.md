...
- U盘一只，用来创建文件系统挂载到 /opt, 因为大多数路由器的存储空间很小。

### 预备知识

您至少须具备Linux基础知识，并且能通过ssh登陆OpenWrt.
除非有特别说明，本文所有命令都用root身份执行，命令中需要替换的部分用尖括号`<>`标注。
OpenWrt从25.12版本起改用 [apk](https://wiki.alpinelinux.org/wiki/Alpine_Package_Keeper)
作为软件包管理器，本文的软件安装均用apk操作，未升级的opkg系统请自行升级。

国内镜像仓库可极大提高软件下载速度，下面换用 [科大镜像](https://mirrors.ustc.edu.cn/help/openwrt.html)：

    cp /etc/apk/repositories.d/distfeeds.list /etc/apk/repositories.d/distfeeds.bak
...
先来安装一些软件：
    ...
    apk update
    apk add block-mount e2fsprogs fdisk kmod-fs-ext4 kmod-usb-storage-uas
...
用 `uname -m` 命令查看一下设备的处理器架构，然后去 [这个页面](https://github.com/Entware/Entware/wiki/Alternative-install-vs-standard) 下载对应的安装脚本。
绝大多数路由器应该用 `https://bin.entware.net/armv7sf-k3.2/installer/generic.sh`. 然后安装 Entware, 下载有点慢，耐心等待：）
...
换用 [北外镜像](https://mirrors.bfsu.edu.cn/help/entware/):
...
创建文件夹并修改权限：
    ...
    chown janedoe:jandoe /opt/home/janedoe
...
安装 amule:
    ...
    apk add amule-daemon amule-utils
...
上面两个命令会在系统启动/停止时自动执行。若不想让电骡随系统启动，执行：
...
同时，要确保主路由器的防火墙挡住了从外网到amule远程端口的连接，默认端口为 4711 和 4712。
...
以上即是设置每天5点重启电骡。其它定时设定请查阅 [crontab说明文档](https://pubs.opengroup.org/onlinepubs/007904975/utilities/crontab.html).
